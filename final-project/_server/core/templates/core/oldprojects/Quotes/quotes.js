document.addEventListener('DOMContentLoaded', () => {
    ...
    function togglePin(quote) {
      ...
      const isPinned = pinnedQuotes.some((pinnedQuote) => pinnedQuote.author.toLowerCase() === quote.author.toLowerCase() && pinnedQuote.content.toLowerCase() === quote.content.toLowerCase());
  
      if (isPinned) {
        // Quote is already pinned, so remove it
        pinnedQuotes = pinnedQuotes.filter((pinnedQuote) => !(pinnedQuote.author.toLowerCase() === quote.author.toLowerCase() && pinnedQuote.content.toLowerCase() === quote.content.toLowerCase()));
      } else {
        // Quote is not pinned, so add it to the beginning of the pinned quotes if not already there
        if (!pinnedQuotes.some((pinnedQuote) => pinnedQuote.author.toLowerCase() === quote.author.toLowerCase() && pinnedQuote.content.toLowerCase() === quote.content.toLowerCase())) {
